...
|---|---|---|
| **[GitEngine](https://github.com/brickster241/GitEngine)** | Git rebuilt from first principles in Go — 17 commands, three-way merge (diff3), rebase | Byte-for-byte SHA parity with native git, enforced by a differential test suite; 2× git's bulk ingest, 28.5K obj/s fsck |
| **[PulseHTTP](https://github.com/brickster241/PulseHTTP)** | Production-shaped HTTP/1.1 stack on raw TCP in Go, zero dependencies — router, auth, rate limiting, LRU+ETag cache, TLS, keep-alive proxy | 179K req/s @ p99 2.2 ms, measured by its own load harness; 39-test raw-TCP conformance suite |
| **[JSON-Lexer-Parser](https://github.com/brickster241/JSON-Lexer-Parser-From-Scratch)** | JSON lexer + recursive-descent parser in pure Python, typed errors, bounded depth | 98-case differential run vs the RFC-strict stdlib: 0 disagreements; 572K tokens/s |
